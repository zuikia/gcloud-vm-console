...
const PHRASE_PAIRS = [
  ...
  ["已切换到", "Switched to "],
  ["数据已同步", "Data synced"],
  ["等待同步资源", "Waiting for resource sync"],
  ["Labels JSON 格式无效，请输入 JSON 对象，例如 {\"env\":\"prod\"}。", "Labels JSON is invalid. Enter a JSON object, for example {\"env\":\"prod\"}."],
  ["实例当前为 ", "The instance is currently "],
  ["，请先启动实例后再执行维护操作。", ". Start the instance before running maintenance."],
  ["实例运行状态未知，请先刷新实例状态。", "The instance state is unknown. Refresh the instance status first."],
  ["凭据", "Credentials"],
  ["已生成（不会在记录中显示）", "Generated (not stored in the record)"],
  ...
  ["脚本会保存到本地记录和云端实例 metadata，并在每次启动时以 root 执行。请保证可重复执行，且不要包含密钥。创建成功不代表脚本执行成功。", "The script is saved locally and in cloud instance metadata, and runs as root on each boot. Make it safe to rerun and keep secrets out. VM creation does not verify script success."],
  ["输入可重复执行的启动脚本；不要写入密码、私钥或 token", "Enter an idempotent startup script; do not include passwords, private keys, or tokens"],
  ["自定义脚本不能为空，请填写启动脚本。", "Enter a startup script before continuing."],
  ["自定义脚本不能超过 64 KiB。", "The startup script must not exceed 64 KiB."],
  ["自定义脚本随实例启动执行", "Custom script runs on VM boot"],
  ["自定义脚本由实例启动时的 metadata 流程执行；请检查云端日志，不使用节点流水线。", "The script runs through VM startup metadata. Check cloud logs; no node pipeline is used."],
  ["启动脚本会写入云端 metadata，并在每次启动时以 root 执行；请勿包含密钥。控制台不会自动验证脚本结果、修改 SSH 或同步脚本所需端口。", "The script is stored in cloud metadata and runs as root on each boot; keep secrets out. The console does not verify script results, change SSH, or sync script ports."],
  ["控制台不会自动修改 sshd", "the console does not modify sshd automatically"],
  ["每次启动时执行", "runs on each boot"],
  ["当前预览包含本版本尚未自动执行的修改；可以查看影响，但执行按钮会保持禁用。请使用“基于此新建”生成替换实例，或手动完成这些修改。", "This preview includes changes this version cannot execute automatically. Execution stays disabled. Use \"Create from this\" for a new instance, or make these changes manually."],
  ["更新实例标签（当前版本仅预览）", "Update instance labels (preview only)"],
  ["更新网络标签（当前版本仅预览）", "Update network tags (preview only)"],
  ["更新启动脚本标记（当前版本仅预览）", "Update startup script marker (preview only)"],
  ["调整机器类型（当前版本仅预览）", "Change machine type (preview only)"],
  ["调整磁盘大小（当前版本仅预览）", "Resize disk (preview only)"],
  ["更换系统镜像（当前版本仅预览）", "Change system image (preview only)"],
  ["更换磁盘类型（当前版本仅预览）", "Change disk type (preview only)"],
  ["调整公网 IP 模式（当前版本仅预览）", "Change public IP mode (preview only)"],
  ["调整网络层级（当前版本仅预览）", "Change network tier (preview only)"],
  ["调整网卡类型（当前版本仅预览）", "Change NIC type (preview only)"],
  ["调整静态地址（当前版本仅预览）", "Change static address (preview only)"],
  ["调整 VPC 网络（当前版本仅预览）", "Change VPC network (preview only)"],
  ["调整子网（当前版本仅预览）", "Change subnet (preview only)"],
  ["调整服务账号（当前版本仅预览）", "Change service account (preview only)"],
  ["当前配置没有可用账号，请先在本机完成 gcloud 登录。", "No account is available. Sign in with gcloud on this machine first."],
  ["已选择其他项目，请切换", "Different context selected; apply to switch"],
  ["已选择其他 gcloud 配置，请等待项目列表读取完成。", "A different gcloud configuration is selected. Wait for its project list."],
  ["已选择 ", "Selected "],
  ["。点击“切换到此项目”后才会读取该项目的资源。", ". Click \"Switch to this project\" to load its resources."],
  ["重新读取实例清单失败", "Inventory reload failed"],
  ...
  ["状态异常", "Unhealthy"],
  ["状态未知", "Unknown state"],
  // Composite resource/detail strings must win over the shorter word pairs
  // below.  These values are assembled by view-models before the DOM pass;
  // translating only "实例"/"规则"/"状态" produces mixed output such as
  // "InstancesRulesOverexposed" in the English resource detail view.
  ["实例规则过度开放", "Instance rules overexposed"],
  ["实例状态已读取", "Instance status is available"],
  ["可以修改当前部署，或基于此配置新建实例。", "You can edit the current deployment or create a new instance from this configuration."],
  ["修改部署", "Edit deployment"],
  ["最新探测失败", "Latest probe failed"],
  ["项运行中", "running"],
  ["台实例", "instances"],
  ["自有规则待治理", "owned rules need attention"],
  ["条外部规则仅提供人工建议", "external rules are advisory only"],
  ["历史验证", "Historical verification"],
  ["证据状态", "Evidence status"],
  ["最新尝试", "Latest attempt"],
  ["识别证据", "Recognition evidence"],
  ["验证证据", "Verification evidence"],
  ["处理边界", "Action boundary"],
  ["影响范围", "Impact scope"],
  ["目标标签", "Target tag"],
  ["规则缺失", "Rules missing"],
  ["防火墙规则缺失", "Firewall rules missing"],
  ["等高风险操作，需要二次确认", "and other high-risk actions require a second confirmation"],
  ["已启用", "Enabled"],
  ["最新", "Latest"],
  ["端口", "port"],
  ["检查于", "Checked "],
